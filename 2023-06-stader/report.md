...
|:-----------:|:----:|:----------------------------------------------------------------------------------|:-------:|:--------------------------------------------------------------------------------:|
|    High     | H-01 | ```VaultProxy``` implementation can be initialized by anyone and self-destructed  |    5    |                                  VaultProxy.sol                                  |
...
| C4 Severity | REF  | Name                                                                                                                            | Wardens |                                     contract                                     |
|:-----------:|:----:|:--------------------------------------------------------------------------------------------------------------------------------|:-------:|:--------------------------------------------------------------------------------:|
|   Medium    | M-01 | Risk of losing admin access if ```updateAdmin``` set with same current admin address                                            |    3    |                                 StaderConfig.sol                                 |
|   Medium    | M-02 | ```pause/unpause``` functionalities not implemented in many pausable contracts                                                  |   16    | SocializingPool.sol, StaderOracle.sol, OperatorRewardsCollector.sol, Auction.sol |
|   Medium    | M-03 | Stader OPERATOR is a single point of failure                                                                                    |    1    |                          PermissionlessNodeRegistry.sol                          |
|   Medium    | M-04 | ```updatePoolAddress``` functions always revert when updating existing ```poolId```                                             |    4    |                                  PoolUtils.sol                                   |
|   Medium    | M-05 | ```StaderOracle``` - Strict equal can cause no consensus if trusted nodes are removed before consensus                          |    1    |                                 StaderOracle.sol                                 |
|   Medium    | M-06 | Protocol will not benefit from slashing mechanism when remaining penalty bigger than ```minThreshold```                         |    3    |                           ValidatorWithdrawalVault.sol                           |
|   Medium    | M-07 | MEV bots can win all the auctions when ```Auction``` is paused                                                                  |    2    |                                   Auction.sol                                    |
|   Medium    | M-08 | Corruption of oracle data                                                                                                       |    1    |                                 StaderOracle.sol                                 |
|   Medium    | M-09 | ```depositETHOverTargetWeight()``` malicious modifications ```poolIdArrayIndexForExcessDeposit```                               |    2    |                                 PoolSelector.sol                                 |
|   Medium    | M-10 | Owner in ```VaultProxy.sol``` is ```address(0)```                                                                               |    6    |                                  VaultProxy.sol                                  |
|   Medium    | M-11 | ```ValidatorWithdrawalVault.distributeRewards``` can be called to make operator slashable                                       |    1    |                           ValidatorWithdrawalVault.sol                           |
|   Medium    | M-12 | ```ValidatorWithdrawalVault.settleFunds``` doesn’t check amount that user has inside ```NodeELRewardVault``` to pay for penalty |    1    |                           ValidatorWithdrawalVault.sol                           |
|   Medium    | M-13 | No bidder has incentive to bid in the Auction except doing last-minute ```MEV``` due to fixed ```endBlock```                    |    6    |                                   Auction.sol                                    |
|   Medium    | M-14 | Chainlink’s ```latestRoundData``` may return a stale or incorrect result                                                        |   25    |                                 StaderOracle.sol                                 |
